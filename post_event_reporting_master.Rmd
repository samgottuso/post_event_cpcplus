...
comparison_same_type<-rbind(last_event)

##variables for our inline text

survey_results<-'better'
  
survey_responses<-'more'
...
This section looks at `r last_event$Event_name` which took place on `r as.Date.POSIXct(as.numeric(last_event$Date))`. It had `r last_event$Attendance` attendees with `r last_event$Attendance_rate ` of registrants attending. 
...
```
This graph shows that this event had a `r attendance` attendance than the previous event. 
